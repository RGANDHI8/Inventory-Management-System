...
if __name__=="__main__":                    
    ...
    self.category_table=ttk.Treeview(cat_frame,columns=("eid","name"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                messagebox.showerror("Error","Category name should is required",parent=self.root)
            
            else:
                cur.execute("Select * from category where name=?",(self.var_name.get(),))
                row = cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category already present, try using different id")
                else:
